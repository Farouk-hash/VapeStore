    ...
    <div id="editDeviceFormContainer" class="card mb-4">
        <div class="card-header bg-warning text-white d-flex justify-content-between align-items-center">
            <h4 class="mb-0">
                ...
                اضافه الكارتردج - {{ $brand->name ?? 'العلامة التجارية' }}
            </h4>
            ...
            <form wire:submit.prevent="save">
                ...
                <div class="form-section">
                    ...
                    <div id="variants_container">
                        @foreach($cartidgeVariants as $index => $variant)
                            <div class="variant-row mb-2 p-2 border rounded">
                                <div class="row">
                                    <div class="col-md-3">
                                        <input type="text" class="form-control"
                                               wire:model="cartidgeVariants.{{ $index }}.vaping_style"
                                               placeholder="اسم الفاريانت">
                                        @error('cartidgeVariants.'.$index.'.vaping_style') <span class="text-danger">{{ $message }}</span> @enderror
                                    </div>

                                    <div class="col-md-3">
                                        <input type="text" class="form-control"
                                               wire:model="cartidgeVariants.{{ $index }}.resistance"
                                               placeholder="المقاومة (Ω)">
                                        @error('cartidgeVariants.'.$index.'.resistance') <span class="text-danger">{{ $message }}</span> @enderror
                                    </div>
