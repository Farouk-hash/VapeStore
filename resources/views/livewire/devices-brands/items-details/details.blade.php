            ...
            <div class="card-body device-details-container p-4">
                <div class="row row-sm">
                        ...
                        <div class="preview-pic tab-content">
                            @for ($i = 0; $i < count($device->images); $i++)
                                <div class="tab-pane {{ $i == $activeImageIndex ? 'active' : '' }}" id="pic-{{ $i }}">
                                    <img 
                                    src="{{ URL::asset("storage/devices/{$device->images[$i]->url}") }}" 
                        ...
                        <ul class="preview-thumbnail nav nav-tabs">
                            @for ($i = 0; $i < count($device->images); $i++)
                                <li class="{{ $i == $activeImageIndex ? 'active' : '' }}">
                                    ...
                                    data-target="#pic-{{ $i }}" 
                                    data-toggle="tab">
                                        <img src="{{ URL::asset("storage/devices/{$device->images[$i]->url}") }}" alt="صورة السائل"/>
                                    </a>
                                </li>
                            @endfor 
                        </ul>
                    ...
                    <div class="col-xl-7 col-lg-12 col-md-12 mt-4 mt-xl-0">
                        ...
                        <?php
                            $deviceType = $category_id === 2 ? 'deviceFlavor' : 'deviceColor';
                        ?>
                     
                        @foreach ($itemAttributes as $attr)
                            @if($this->{$attr})

                                @include($this->getViewsMapping()[$attr] , ['device'=>$device, 'item'=>$deviceType,'sk'=>null])
                            @endif
                        @endforeach
