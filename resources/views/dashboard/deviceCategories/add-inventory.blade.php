<!-- Fixed Inventory Modal for {{ $device->name }} -->
<div class="modal fade" id="manageInventory{{ $device->id }}" tabindex="-1" role="dialog"
     aria-labelledby="manageInventoryLabel{{ $device->id }}" aria-hidden="true">
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            ...
            <form action="{{route('devicesCategories.addInventory',[$device->id])}}" method="POST" id="inventoryForm{{ $device->id }}">
                @csrf
                ...
                <div class="modal-body">
                    <div class="alert alert-info">
                        ...
                        <strong>إدارة المخزون للجهاز:</strong> {{ $device->name }} - {{ $device->brand->name ?? 'غير معروف' }}
                    </div>

                    <!-- Inventory Items Container -->
                    <div id="inventoryItems{{ $device->id }}">
                            ...
                            <div class="card-body">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select name="inventories[0][device_flavor_id]" class="form-control">
                                            <option value="">اختر النكهة</option>
                                            @foreach($device->flavors as $flavorComponent)
                                                <option value="{{ $flavorComponent->component->id }}">{{ $flavorComponent->component->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    @endif
                    ...
                    <div class="text-center mb-4">
                        <button type="button" class="btn btn-success" id="addMoreItem{{ $device->id }}">
                                ...
                                <tr>
                                    <td>
                                        ...
                                        @if($inventory->deviceFlavor)
                                            <span class="badge badge-warning">{{ $inventory->deviceFlavor->component->name }}</span>
                                        @endif
