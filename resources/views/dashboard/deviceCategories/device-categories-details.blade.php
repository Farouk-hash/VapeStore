...
<div class="breadcrumb-header justify-content-between">
    <div class="my-auto">
        <div class="d-flex">
            ...
            <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ $categorySlug ?? 'الفئة' }}</span>
        </div>
    </div>
</div>
@endsection
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="text-center mb-4">
                    <h2 class="text-primary">{{ ucfirst($categorySlug) }} Devices</h2>
                    <p class="text-muted">عرض جميع الأجهزة المتاحة في هذه الفئة</p>
                </div>
                ...
                <div class="stats-row">
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <div class="stat-item">
                                <div class="stat-number">{{ $devices->flatMap->colors->count() }}</div>
                                <div class="stat-label">الألوان المتاحة</div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="stat-item">
                                <div class="stat-number">{{ $devices->flatMap->features->count() }}</div>
                                <div class="stat-label">الميزات</div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="stat-item">
                                <div class="stat-number">{{ $devices->flatMap->flavors->count() }}</div>
...
<div id="addDeviceFormContainer" class="card mb-4" style="display: none;">
    <div class="card-header bg-primary text-white">
        <h4 class="mb-0">
            <i class="mdi mdi-plus-circle"></i>
            إضافة جهاز جديد - {{ $devices[0]->brand->name ?? 'العلامة التجارية' }}
        </h4>
    </div>
    <div class="card-body">
        <form id="addDeviceForm">
            @csrf
            <input type="hidden" name="brand_id" value="{{ $devices[0]->brand->id ?? '' }}">
            <input type="hidden" name="category_id" value="{{ $devices[0]->category->id ?? '' }}">
            ...
            <div class="form-section">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="device_model" name="model">
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="device_release_year" name="release_year" 
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="device_sku" name="sku">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                    ...
                    @if($categorySlug == 'disposable')
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="device_puffs">عدد الانفاس</label>
                            <input type="number" class="form-control" id="device_puffs" name="device_puffs" step="1" min="0">
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="device_puffs_nicotine_strength" name="device_puffs_nicotine_strength">
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="device_puffs_nicotine_type" name="device_puffs_nicotine_type" step="1" min="0">
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="device_puffs_ice_type" name="device_puffs_ice_type" placeholder="Ice , Non-Ice , Mixed">
                        </div>
                    </div>
                    @endif
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="device_description" name="description" rows="3"></textarea>
                </div>
            </div>
            ...
            <!-- Features Section -->
            <div class="form-section">
                <h5 class="section-title">الميزات</h5>
                <div class="form-group">
                    <label for="device_features">ميزات الجهاز (اختياري)</label>
                ...
                <div id="features_preview" class="tags-preview"></div>
            </div>

            <!-- Flavors Section -->
            <div class="form-section">
                <h5 class="section-title">النكهات المتاحة</h5>
                <div class="form-group">
                    <label for="device_flavors">النكهات المتوفرة (اختياري)</label>
                ...
                <div id="flavors_preview" class="tags-preview"></div>
            </div>
                ...
                <div id="devicesContainer">
                    @forelse($devices as $device)
                        <div class="device-card" data-device-id="{{ $device->id }}">
                            <div class="device-header">
                                ...
                                <div class="device-release-year">{{ $device->release_year ?? 'N/A' }}</div>
                                <small>{{ $device->category->description ?? 'جهاز متطور للفيب' }}</small>
                            </div>
                            ...
                            <div class="device-options">
                                ...
                                @if($device->speces && $device->speces->count() > 0)
                                    <div class="collapsible-section">
                                        <div class="collapsible-header"  data-target="#specs-{{ $device->id }}">
                                            <h5>
                                                <i class="mdi mdi-cog"></i> المواصفات التقنية
                                            </h5>
                                            <div class="collapsible-toggle">
                                                <span>{{ $device->speces->count() }} مواصفة</span>
                                                @if($device->speces->count() > 3)
                                                    <div class="items-preview">
                                                        @foreach($device->speces->take(2) as $spec)
                                                            <span class="preview-item">{{ Str::limit($spec->spec_key, 10) }}</span>
                                                        @endforeach
                                                        <span class="preview-more">+{{ $device->speces->count() - 2 }}</span>
                                                    </div>
                                                @endif
                                                <i class="mdi mdi-chevron-down collapsible-arrow"></i>
                                            </div>
                                        </div>
                                        <div class="collapsible-content" id="specs-{{ $device->id }}">
                                ...
                                @if($device->colors && $device->colors->count() > 0)
                                    <div class="collapsible-section">
                                        <div class="collapsible-header"  data-target="#colors-{{ $device->id }}">
                                            <h5>
                                                <i class="mdi mdi-palette"></i> الألوان المتاحة
                                          ن  </h5>
                                            <div class="collapsible-toggle">
                                                <span>{{ $device->colors->count() }} لون</span>
                                                @if($device->colors->count() > 3)
                                                    <div class="items-preview">
                                                        @foreach($device->colors->take(3) as $color)
                                                            <span class="preview-item">{{ $color->name }}</span>
                                                        @endforeach
                                                        @if($device->colors->count() > 3)
                                                            <span class="preview-more">+{{ $device->colors->count() - 3 }}</span>
                                                        @endif
                                                    </div>
                                                @endif
                                                <i class="mdi mdi-chevron-down collapsible-arrow"></i>
                                        ...
                                        <div class="collapsible-content" id="colors-{{ $device->id }}">
                                ...
                                @if($device->features && $device->features->count() > 0)
                                    <div class="collapsible-section">
                                        <div class="collapsible-header"  data-target="#features-{{ $device->id }}">
                                            <h5>
                                                <i class="mdi mdi-star"></i> الميزات الرئيسية
                                            </h5>
                                            <div class="collapsible-toggle">
                                                <span>{{ $device->features->count() }} ميزة</span>
                                                @if($device->features->count() > 2)
                                                    <div class="items-preview">
                                                        @foreach($device->features->take(2) as $feature)
                                                            <span class="preview-item">{{ Str::limit($feature->name, 10) }}</span>
                                                        @endforeach
                                                        @if($device->features->count() > 2)
                                                            <span class="preview-more">+{{ $device->features->count() - 2 }}</span>
                                                        @endif
                                                    </div>
                                                @endif
                                                <i class="mdi mdi-chevron-down collapsible-arrow"></i>
                                        ...
                                        <div class="collapsible-content" id="features-{{ $device->id }}">
                                ...
                                <!-- Flavors Section (Collapsible) -->
                                @if($device->flavors && $device->flavors->count() > 0)
                                    <div class="collapsible-section">
                                        <div class="collapsible-header"  data-target="#flavors-{{ $device->id }}">
                                            <h5>
                                                <i class="mdi mdi-cup"></i> النكهات المتاحة
                                            </h5>
                                            <div class="collapsible-toggle">
                                                <span>{{ $device->flavors->count() }} نكهة</span>
                                                @if($device->flavors->count() > 3)
                                                    <div class="items-preview">
                                                        @foreach($device->flavors->take(3) as $flavorComponennt)
                                                            <span class="preview-item">{{ Str::limit($flavorComponennt->component->name, 8) }}</span>
                                                        @endforeach
                                                        @if($device->flavors->count() > 3)
                                                            <span class="preview-more">+{{ $device->flavors->count() - 3 }}</span>
                                                        @endif
                                                    </div>
                                                @endif
                                                <i class="mdi mdi-chevron-down collapsible-arrow"></i>
                                        ...
                                        <div class="collapsible-content" id="flavors-{{ $device->id }}">
                                            <div class="flavors-grid">
                                                @foreach($device->flavors as $componentFlavor)
                                                    <div class="flavor-item">
                                                        {{ $componentFlavor->component->name }}
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
                                ...
                                <!-- Puffs Section (Collapsible) -->
                                @if($device->puffs && $device->puffs->count() > 0)
                                    <div class="collapsible-section">
                                        <div class="collapsible-header" data-target="#puffs-{{ $device->id }}">
                                            <h5>
                                                <i class="mdi mdi-smoking"></i> الانفاس المتاحة
                                            </h5>
                                            <div class="collapsible-toggle">
                                                <span>{{ $device->puffs->count() }} نفس</span>
                                                @if($device->puffs->count() >= 2)
                                                    <div class="items-preview">
                                                        @foreach($device->puffs->take(2) as $puff)
                                                            <span class="preview-item">
                                                                {{ $puff->value }}Puff --{{$puff->nicotine_strength}} 
                                                            </span>
                                                        @endforeach
                                                        @if($device->puffs->count() > 3)
                                                            <span class="preview-more">+{{ $device->puffs->count() - 3 }}</span>
                                                        @endif
                                                    </div>
                                                @endif
                                                <i class="mdi mdi-chevron-down collapsible-arrow"></i>
                                        ...
                                        <div class="collapsible-content" id="puffs-{{ $device->id }}">
                                            <div class="flavors-grid">
                                                @foreach($device->puffs as $puff)
                                                    <div class="flavor-item">
                                                        {{ $puff->value }} Puff
                                                        <br>
                                                        {{ $puff->nicotine_strength }}{{ $puff->nicotine_type }} {{$puff->ice_type}}
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
                                ...
                                <div class="device-actions">
                                    <button class="device-action-btn btn-edit" data-toggle="modal" data-target="#editDevice{{ $device->id }}">
                                        <i class="fas fa-edit"></i>
                                        تعديل
                                    </button>
                                    
                                    <button class="device-action-btn btn-inventory" data-toggle="modal" data-target="#manageInventory{{ $device->id }}">
                        ...
                        window.deviceData[{{ $device->id }}] = {
                            ...
                            flavors: [
                                @if($device->flavors && $device->flavors->count() > 0)
                                    @foreach($device->flavors as $flavorComponent)
                                        {
                                            id: {{ $flavorComponent->component->id }},
                                            name: @json($flavorComponent->component->name)
                                        },
                                    @endforeach
                                @endif
                            ]
                    ...
                    @empty
                        <div class="alert alert-info text-center">
                            <h5>لا توجد أجهزة متاحة</h5>
                            <p>لم يتم إضافة أي أجهزة في هذه الفئة بعد.</p>
                        </div>
                    @endforelse
                </div>
...
$(document).ready(function() {
        const config = {
        routes: {
            addDevice: '{{ route("devicesCategories.store") ?? "#" }}',
    ...
    $('#device_features').on('input', function() {
        updateTagsPreview('device_features', 'features_preview');
    });
...
$(document).ready(function() {
    ...
    $(document).on('submit', '#addDeviceForm', function(e) {
        ...
        const features = $('#device_features').val().split(/[,،;؛]/).map(f => f.trim()).filter(f => f);
        const flavorIds = $('.select2-flavors').val() || [];

        formData.append('flavor_ids', JSON.stringify(flavorIds));        
        formData.append('colors_array', JSON.stringify(colors));
        formData.append('features_array', JSON.stringify(features));
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                $('#addDeviceBtn').html('<i class="mdi mdi-plus-circle"></i> إضافة جهاز جديد');
    ...
    function createNewInventoryItem(index) {
        ...
        if (deviceInfo.colors && deviceInfo.colors.length > 0) {
            ...
            deviceInfo.colors.forEach(color => {
                html += `<option value="${color.tempId}">${color.name}</option>`;
            });
        ...
        // Add flavor selection if device has flavors
        if (deviceInfo.flavors && deviceInfo.flavors.length > 0) {
            html += `
                        <div class="col-md-6">
                            <label class="font-weight-bold text-primary">النكهة (اختياري)</label>
                            <select name="inventories[${index}][device_flavor_id]" class="form-control">
                                <option value="">اختر النكهة</option>`;
            
            deviceInfo.flavors.forEach(flavor => {
                html += `<option value="${flavor.id}">${flavor.name}</option>`;
            });
