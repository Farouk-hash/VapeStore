...
<div class="breadcrumb-header justify-content-between">
    <div class="my-auto">
        <div class="d-flex">
            <h4 class="content-title mb-0 my-auto">الخرطوشه</h4>
            <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ الخرطوشه</span>
        </div>
    </div>
</div>
@endsection
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="text-center mb-4">
                    <h2 class="text-primary">الخرطوشه</h2>
                    <p class="text-muted">عرض جميع الخرطوشه المتاحة</p>
                </div>
                ...
                <div class="stats-row">
                    <div class="row">
                        <div class="col-md-4">
                            <div class="stat-item">
                                <div class="stat-number">{{ $cartridges->count() }}</div>
                                <div class="stat-label">إجمالي الخرطوشه</div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="stat-item">
                                <div class="stat-number">{{ $cartridges->flatMap->variants->count() }}</div>
...
<div id="addDeviceFormContainer" class="card mb-4" style="display: none;">
    <div class="card-header bg-primary text-white">
        <h4 class="mb-0">
            <i class="mdi mdi-plus-circle"></i>
            إضافة خرطوشة جديد - {{ $cartridges[0]->brand->name ?? 'العلامة التجارية' }}
        </h4>
    </div>
    ...
    <div class="card-body">
        <form id="addDeviceForm">
            @csrf
            <input type="hidden" name="brand_id" value="{{ $cartridges[0]->brand->id ?? '' }}">
            <input type="hidden" name="category_id" value="{{ $cartridges[0]->category->id ?? '' }}">
            ...
            <div class="form-section">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label for="cartridge_name">اسم الخرطوشة *</label>
                        <input type="text" class="form-control" id="cartridge_name" name="name" required>
                    </div>

                    <div class="col-md-6 mb-3">
                        <label class="form-label">نوع الخرطوشة *</label>
                    ...
                    <div class="col-md-6 mb-3">
                        <label for="capacity_ml">السعة (مل) *</label>
                        <input type="number" step="0.1" class="form-control" id="capacity_ml" name="capacity_ml" required>
                    </div>

                    <div class="col-md-6 mb-3">
                        <label class="form-label">مادة الخرطوشة *</label>
                    ...
                    <div class="col-md-6 mb-3">
                        <label class="form-label">نوع الكويل *</label>
                        <select class="form-control" name="coil_type">
                            <option value="">اختر نوع الكويل</option>
                ...
                <div class="form-group">
                    <label for="cartridge_description">الوصف</label>
                    <textarea class="form-control" id="cartridge_description" name="description" rows="3"></textarea>
                </div>
            </div>

            <!-- Toggle for resistance -->
            <div class="form-check form-switch mb-4">
                <input class="form-check-input" type="checkbox" id="hasResistance" name="has_resistance" checked>
                <label class="form-check-label" for="hasResistance">هل تحتوي الخرطوشة على مقاومات (Ohms)؟</label>
            </div>

            <!-- Coil Variants Section -->
        ...
        <div class="form-section">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="number" class="form-control" name="coils[0][resistance]" placeholder="Ωمثال: 0.15" step="0.01" min="0.1" >
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">مدى الواط (Wattage) *</label>
                                <input type="text" class="form-control" name="coils[0][wattage_range]" placeholder="Wمثال: 60-75W" >
                            </div>

                            <div class="col-md-6">
                                <label class="form-label">نمط التدخين *</label>
                                <select class="form-control" name="coils[0][vaping_style]" >
            ...
            <!-- Images Section -->
            <div class="form-section">
                <h5 class="section-title">صور الخرطوشة</h5>
                <div class="form-group">
                    <input type="file" class="form-control-file" id="cartridge_images" name="images[]" multiple accept="image/*">
            ...
            <div class="form-actions mt-4">
                <button type="submit" class="btn btn-primary">
                    <i class="mdi mdi-content-save"></i> حفظ الخرطوشة
                </button>
                ...
                <div id="devicesContainer">
                    @forelse($cartridges as $cartridge)
                        <div class="device-card" data-device-id="{{ $cartridge->id }}">
                            <div class="device-header">
                                <h3 class="device-title">{{ $cartridge->name }}--{{$cartridge->type}}</h3>
                                <h4>{{ $cartridge->material }} -- <span style="color:blue;">{{$cartridge->coil_type ?? 'لا يوجد نوع محدد'}}</span></h4> 
                                <small>{{ $cartridge->description ?? 'جهاز متطور للفيب' }}</small>                           
                            </div>
                            ...
                            <div class="device-options">
                                ...
                                @if($cartridge->variants && $cartridge->variants->count() > 0)
                                    <div class="collapsible-section">
                                        <div class="collapsible-header"  data-target="#specs-{{ $cartridge->id }}">
                                            <h5>
                                                <i class="mdi mdi-cog"></i> المواصفات التقنية
                                            </h5>
                                            <div class="collapsible-toggle">
                                                <span>{{ $cartridge->variants->count() }} مقاومات</span>
                                                @if($cartridge->variants->count() > 1)
                                                    <div class="items-preview">
                                                        @foreach($cartridge->variants->take(2) as $variant)
                                                            <span class="preview-item">{{ $variant->resistance }}Ω</span>
                                                        @endforeach
                                                        <span class="preview-more">+{{ $cartridge->variants->count() - 2 }}</span>
                                                    </div>
                                                @endif
                                                <i class="mdi mdi-chevron-down collapsible-arrow"></i>
                                            </div>
                                        </div>
                                        <div class="collapsible-content" id="specs-{{ $cartridge->id }}">
                                            <div class="specs-container">
                                                <div class="spec-grid">
                                                    @foreach($cartridge->variants as $variant)
                                                        <div class="spec-item">
                                                            <span class="spec-value">{{ $variant->resistance }}Ω</span>
                                                            <div class="spec-label">{{ $variant->vaping_style }}</div>
                                                        </div>
                                                    @endforeach
                                                </div>
                                            </div>
                                ...
                                <div class="device-actions">
                                    <button class="device-action-btn btn-edit" data-toggle="modal" data-target="#editDevice{{ $cartridge->id }}">
                                        <i class="fas fa-edit"></i>
                                        تعديل
                                    </button>
                                    
                                    <button class="device-action-btn btn-inventory" data-toggle="modal" data-target="#manageInventory{{ $cartridge->id }}">
                        ...
                        @include('dashboard.cartridges.add-inventory') 

                        <script>
                        // Store device data in JavaScript for dynamic use (THIS GOES INSIDE THE DEVICE LOOP)
                        window.deviceData = window.deviceData || {};
                        window.deviceData[{{ $cartridge->id }}] = {
                            colors: [
                                @if($cartridge->variants && $cartridge->variants->count() > 0)
                                    @foreach($cartridge->variants as $variant)
                                        {
                                            id: {{ $variant->id }},
                                            value: @json($variant->resistance),
                                            vapingStyle:@json($variant->vaping_style),
                                        },
                                    @endforeach
                                @endif
                            ],
...
$(document).ready(function() {
        const config = {
        routes: {
            addCartridge: '{{ route("cartridges.store") ?? "#" }}',
...
$(document).ready(function() {
    ...
    function createNewCoil(index) {
        ...
        return `
            <div class="coil-item card mb-4" data-coil-index="${index}">
                <div class="card-header bg-light d-flex justify-content-between align-items-center">
                    <h6 class="mb-0">خرطوشه #${coilNumber}</h6>
                    <button type="button" class="btn btn-sm btn-danger remove-coil">
                ...
                <div class="card-body">
                    <div class="row mb-3">
                            <div class="col-md-6">
                                <label class="form-label">المقاومة (Ω) *</label>
                                <input type="number" class="form-control" name="coils[${index}][resistance]" placeholder="مثال: 0.15" step="0.01" min="0.1" required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">مدى الواط (Wattage) *</label>
                                <input type="text" class="form-control" name="coils[${index}][wattage_range]" placeholder="مثال: 60-75W" required>
                            </div>
    ...
    function submitCoilForm(formData) {
        ...
        $.ajax({
            url: config.routes.addCartridge, // Replace with your actual endpoint
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
