...
<div class="breadcrumb-header justify-content-between">
    <h4 class="content-title mb-0">إدارة ماركات الكويلز</h4>
</div>
<!-- breadcrumb -->
@endsection
...
<div class="row row-sm">
    <div class="col-xl-12">
        <div class="card">
            ...
            <div class="card-header pb-0">
                <div class="d-flex justify-content-between">
                    <h4 class="card-title mg-b-0">قائمة ماركات الكويلز</h4>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table text-md-nowrap" id="example1">
                        ...
                        <tbody>
                            @foreach ($coilBrands as $coilBrand)
                                <tr>
                                    <td>{{ $coilBrand->name }}</td>
                                    <td>{{ $coilBrand->country }}</td>
                                    <td>
                                        @if($coilBrand->website)
                                            <a href="{{ $coilBrand->website }}" target="_blank">{{ $coilBrand->website }}</a>
                                        @else
                                            <span class="text-muted">—</span>
                                        @endif
                                    </td>
                                    <td>{{ substr($coilBrand->description, 0, 30)}}</td>
                                    <td>{{$coilBrand->coils_count}}</td>
                                    <td>
                                        <div class="dropdown">
                                            <button class="btn btn-sm btn-primary dropdown-toggle" 
                                                    type="button" id="dropdownMenuButton{{ $coilBrand->id }}" 
                                                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                الإجراءات
                                            </button>
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton{{ $coilBrand->id }}">
                                                <a class="dropdown-item modal-effect text-primary" 
                                                   {{-- href="{{ route('tank-brands.edit', $coilBrand->id) }}" --}}
                                                   >
                                                    <i class="las la-pen"></i> تعديل
                                                </a>

                                                <form 
                                                {{-- action="{{ route('tank-brands.destroy', $coilBrand->id) }}"  --}}
                                                ...
                                                <a class="dropdown-item modal-effect text-info" 
                                                   href="{{ route('coils.show', $coilBrand->id) }}"
                                                   >
